...
public class OrderController{
	...
	public String confirm(ModelMap map,String oid){
		...
		orderService.updateStatus(oid, 6);//设置状态为等待评价
//		orderService.updateStatus(oid, 4);//设置状态为确认收货
	...
	public String pingjia(ModelMap map,String oid,String fenshu){
		orderService.updateStatus(oid, 4);//设置状态为等待评论
		
		List<OrderItem> orderItemList = orderItemDao.findByOid(oid);
		for (int i = 0; i < orderItemList.size(); i++) {
			String bid = orderItemList.get(i).getBook().getBid();
			try {
				Book book = bookDao.findByBid(bid);
				String score = book.getScore();
				if(score != null){
					String[] scores = score.split("\\|");
					int totalAvg = Integer.parseInt(scores[0]) + Integer.parseInt(fenshu);
					int person = Integer.parseInt(scores[1]) + 1;
					book.setScore(totalAvg+"|"+person);
				}else{
					book.setScore(fenshu+"|"+1);
				}
				...
				BigDecimal b6 = new BigDecimal(book.getView());
				...
				map1.put("bid", bid);
				map1.put("score", book.getScore());
				map1.put("total", res);
				bookDao.editScore(map1);
		...
		map.addAttribute("code", "success");
		map.addAttribute("msg", "您的订单已评论成功！");
		return "/jsps/msg";
	}
	...
	public String reList(HttpServletRequest request,String oid){
		System.out.println("liangl"+oid);
		orderService.updateOid(oid);
		return "forward:/order/myOrders.do";
	}
}
